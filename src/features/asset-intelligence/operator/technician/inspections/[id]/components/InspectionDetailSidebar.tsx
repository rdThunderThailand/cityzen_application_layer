...
export function InspectionDetailSidebar({ detail }: { detail: TechnicianInspectionDetail }) {
  const steps = [
    { label: "รับงานแล้ว", date: "20 พ.ค. 2567 09:15", active: true },
    { label: "กำลังเดินทาง", date: "", active: false },
    { label: "ถึงหน้างาน", date: "", active: false },
    { label: "ดำเนินการแก้ไข", date: "", active: false },
    { label: "สรุปผลและแนบหลักฐาน", date: "", active: false },
    { label: "ส่งตรวจรับ", date: "", active: false },
  ];
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-white rounded-[16px] border border-slate-200 shadow-sm p-8 xl:p-12 lg:p-8">
        ...
        <div className="flex flex-col gap-6">
          <div className="flex justify-between items-start gap-4">
            ...
            <span className="text-[13px] font-bold text-slate-800 text-right">{detail.woNumber}</span>
          </div>
          <div className="flex justify-between items-start gap-4">
            <span className="text-[13px] font-bold text-slate-500 shrink-0">ประเภทงาน</span>
            <span className="text-[13px] font-bold text-slate-800 text-right">{detail.taskType}</span>
          </div>
          ...
          <div className="flex justify-between items-start gap-4">
            ...
            <span className="text-[13px] font-bold text-slate-800 text-right">{detail.woDate}</span>
          </div>
          <div className="flex justify-between items-start gap-4">
            <span className="text-[13px] font-bold text-slate-500 shrink-0">ผู้แจ้ง</span>
            <span className="text-[13px] font-bold text-slate-800 text-right">{detail.reporterName} ({detail.reporterDept})</span>
          </div>
          <div className="flex justify-between items-start gap-4">
            <span className="text-[13px] font-bold text-slate-500 shrink-0">สถานที่</span>
            <span className="text-[13px] font-bold text-slate-800 text-right max-w-[160px]">{detail.location}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[16px] border border-slate-200 shadow-sm p-8 xl:p-12 lg:p-8">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[15px] font-black text-blue-900">{detail.progressPercent}%</span>
        </div>

        {/* Horizontal Progress Bar */}
        <div className="w-full h-1.5 bg-slate-100 rounded-full mb-8">
          <div className="h-full bg-[#1D4ED8] rounded-full transition-all duration-500" style={{ width: `${detail.progressPercent}%` }}></div>
        </div>
          ...
          <div className="flex flex-col gap-6 relative z-10">
            {steps.map((step, idx) => (
              <div key={idx} className="flex items-start gap-5">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 z-10 transition-colors ${step.active ? "bg-[#1D4ED8] text-white" : "bg-white text-slate-700 border-[2px] border-[#E2E8F0]"
                  }`}>
                ...
                <div className="flex-1 flex justify-between items-start mt-0.5">
                  <p className={`text-[13px] font-bold ${step.active ? "text-[#1D4ED8]" : "text-slate-700"}`}>
                    {step.label}
                  </p>
                  {step.date && (
                    <p className={`text-[11px] font-medium mt-0.5 ${step.active ? "text-slate-500" : "text-slate-400"}`}>
                      {step.date}
                    </p>
                  )}
                </div>
      ...
      <div className="bg-white rounded-[16px] border border-slate-200 shadow-sm p-8 xl:p-12 lg:p-8 flex flex-col gap-5">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[14px] font-bold text-slate-900 mb-0.5">{detail.reporterName} ({detail.reporterDept})</p>
            <p className="text-[12px] font-bold text-slate-500">เบอร์โทร: {detail.reporterPhone}</p>
          </div>
        </div>
