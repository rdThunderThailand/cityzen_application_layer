...
export function InspectionPendingAcceptanceView({ detail, onBack, onReturnToHome, onApprovalComplete }: InspectionPendingAcceptanceViewProps) {
  ...
  }, [showSuccessNotification, countdown, onApprovalComplete]);
          ...
          <div className="bg-white rounded-[16px] border border-slate-200 shadow-sm p-8 xl:p-12 flex flex-col gap-6">
            ...
            <div className="flex items-center justify-between relative px-4">
              ...
              {[
                ...
                { id: 6, label: "รอตรวจรับ", date: "ส่งเมื่อ 20 พ.ค. 2567 10:35", status: "active", icon: <span className="font-black text-[14px]">6</span> },
              ].map((step, idx) => (
                <div key={idx} className="flex flex-col items-center gap-2 z-10 w-[120px]">
                  <div className={`w-[32px] h-[32px] rounded-full flex items-center justify-center bg-white border-[2px] ${step.status === 'completed' ? 'border-emerald-500 text-emerald-500' :
                      step.status === 'active' ? 'border-amber-400 bg-amber-400 text-white' :
                        'border-slate-200 text-slate-300'
                    }`}>
                    {step.icon}
                  </div>
                  <div className="flex flex-col items-center text-center">
                    <span className={`text-[12px] font-bold ${step.status === 'active' ? 'text-amber-500' : step.status === 'completed' ? 'text-emerald-600' : 'text-slate-400'}`}>
                      {step.label}
                    </span>
                    <span className="text-[10px] font-medium text-slate-400">
                      {step.date.split('ส่งเมื่อ ').map((s, i) => i === 0 && s === '' ? 'ส่งเมื่อ ' : <span key={i}>{s}<br /></span>)}
                    </span>
                  </div>
                </div>
              ))}
                ...
                <div className="flex flex-col gap-5 flex-1">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex justify-between items-center text-[12px]">
                      <span className="font-medium text-slate-500">อุณหภูมิห้อง (°C)</span>
                      <span className="font-bold text-slate-800">24.6</span>
                    </div>
                    <div className="flex justify-between items-center text-[12px]">
                      <span className="font-medium text-slate-500">แรงดันไฟฟ้า (V)</span>
                      <span className="font-bold text-slate-800">219</span>
                    </div>
                    <div className="flex justify-between items-center text-[12px]">
                      <span className="font-medium text-slate-500">กระแสไฟฟ้า (A)</span>
                      <span className="font-bold text-slate-800">4.0</span>
                    </div>
                    <div className="flex justify-between items-center text-[12px]">
                      <span className="font-medium text-slate-500">ความเย็น (°C)</span>
                      <span className="font-bold text-slate-800">11.2</span>
                    </div>
                  </div>
...
function PendingAcceptanceSidebar({ detail, onSimulateApproval }: { detail: TechnicianInspectionDetail, onSimulateApproval: () => void }) {
  const steps = [
    ...
    { label: "รอตรวจรับ", date: "20 พ.ค. 2567 10:35\nรอตรวจรับ", status: "active" },
  ];
            ...
            {steps.map((step, idx) => (
              <div key={idx} className="flex items-center gap-4">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 z-10 transition-colors ${step.status === 'completed' ? "bg-emerald-500 text-white" :
                    step.status === 'active' ? "bg-amber-400 text-white" :
                ...
                <div className="flex-1 flex justify-between items-center">
                  <span className={`text-[12px] font-bold ${step.status === 'active' ? 'text-amber-500' : 'text-slate-800'}`}>
                    {step.label}
                  </span>
                  <span className={`text-[10px] font-medium ${step.status === 'active' ? 'text-slate-500 text-right' : 'text-slate-500'}`}>
                    {step.date.split('\n').map((line, i) => (
                      <span key={i} className={i === 1 ? 'text-amber-500' : ''}>{line}<br /></span>
                    ))}
                  </span>
                </div>
              </div>
