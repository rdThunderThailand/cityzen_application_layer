...
export function InspectionSummaryCostsView({ detail, onTabChange, onBack, onNext }: InspectionSummaryCostsViewProps) {
  ...
  const costs = [
    { id: 1, type: "ค่าบริการล้างทำความสะอาด", desc: "ค่าล้างคอยล์ร้อน - คอยล์เย็น", provider: "บจก. คลีนแอร์ เซอร์วิส", amount: 300.00, doc: "ใบเสร็จ.pdf", docType: "pdf" },
    { id: 2, type: "ค่าขนส่ง / เดินทางพิเศษ", desc: "ค่าขนส่งน้ำยาแอร์ส่วน", provider: "Flash Express", amount: 120.00, doc: "ใบเสร็จ.pdf", docType: "pdf" },
    { id: 3, type: "ค่าธรรมเนียมจอดรถ", desc: "ค่าจอดรถอาคารจอด B", provider: "อาคารจอดรถ", amount: 60.00, doc: "รูปภาพ.jpg", docType: "image" },
  ];
    ...
    <div className="min-h-full flex-1 bg-slate-50 p-8 xl:p-12 w-full pb-40">
      ...
      <div className="flex flex-col xl:flex-row gap-6">
                ...
                <tbody>
                  {costs.map((item) => (
                    <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-4 px-4 text-[12px] font-medium text-slate-500">{item.desc}</td>
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-1.5 text-[12px] font-bold text-blue-600 hover:underline cursor-pointer">
                          {item.docType === 'pdf' ? <FileText className="w-3.5 h-3.5" /> : <ImageIcon className="w-3.5 h-3.5" />}
                          {item.doc}
                        </div>
                      </td>
        ...
        <div className="w-full xl:w-[320px] shrink-0 flex flex-col gap-6">
          <SummaryCostsSidebar detail={detail} />
        </div>
...
function SummaryCostsSidebar({ detail }: { detail: TechnicianInspectionDetail }) {
  const steps = [
